        ...
        private void KillSteal()
        {
            if (Menu.Item("ME").GetValue<bool>())
            {
                if (E.IsReadyPerfectly())
                {
                    ...
                    {
                        var target = ObjectManager.Get<Obj_AI_Hero>().FirstOrDefault(x => x.IsKillableAndValidTarget(E.GetDamage(x), E.DamageType, E.Range) && IsPoisoned(x));
                    ...
                    {
                        var target = ObjectManager.Get<Obj_AI_Hero>().FirstOrDefault(x => x.IsKillableAndValidTarget(E.GetDamage(x), E.DamageType, E.Range));
                        if (target.IsValidTarget(E.Range))
                        {
                            E.CastOnUnit(target);
                        }
