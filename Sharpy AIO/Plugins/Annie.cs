...
namespace Sharpy_AIO.Plugins
{
    public class Annie
    {
        ...
        public Annie()
        {
            Game.PrintChat("Sharpy AIO :: Annie Loaded :)");
        ...
        private void Killsteal()
        {
            if (Menu.Item("MK").GetValue<bool>())
            {
                if (R.IsReadyPerfectly() && !HaveTibbers)
                {
                    var target = ObjectManager.Get<Obj_AI_Hero>().FirstOrDefault(x => x.IsKillableAndValidTarget(R.GetDamage(x), R.DamageType, R.Range));
                    if (target != null)
                    {
                        R.CastOnUnit(target);
                    }
                ...
                else
                {
                    if (Q.IsReadyPerfectly())
                    {
                        var target = ObjectManager.Get<Obj_AI_Hero>().FirstOrDefault(x => x.IsKillableAndValidTarget(Q.GetDamage(x), Q.DamageType, Q.Range) && Q.GetDamage(x) > W.GetDamage(x));
                        if (target != null)
                        {
                            Q.CastOnUnit(target);
                        }
                        else
                        {
                            if (W.IsReadyPerfectly())
                            {
                                var otarget = ObjectManager.Get<Obj_AI_Hero>().FirstOrDefault(x => x.IsKillableAndValidTarget(Q.GetDamage(x), Q.DamageType, Q.Range) && Q.GetDamage(x) < W.GetDamage(x));
                                if (otarget != null)
                                {
                                    W.Cast(otarget);
                                }
                    ...
                    {
                        var target = ObjectManager.Get<Obj_AI_Hero>().FirstOrDefault(x => x.IsKillableAndValidTarget(W.GetDamage(x), W.DamageType, W.Range));
                        if (target != null)
                        {
                            W.Cast(target);
                        }
