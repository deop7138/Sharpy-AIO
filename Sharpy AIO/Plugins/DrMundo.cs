...
namespace Sharpy_AIO.Plugins
{
    public class DrMundo
    {
        ...
        public DrMundo()
        {
            Game.PrintChat("Sharpy AIO :: Dr.Mundo Loaded :)");
        ...
        private void KillSteal()
        {
            if (Menu.Item("MK").GetValue<bool>())
            {
                if (Menu.Item("KEQ").GetValue<bool>())
                {
                    ...
                    {
                        var target = ObjectManager.Get<Obj_AI_Hero>().FirstOrDefault(x => x.IsKillableAndValidTarget(Q.GetDamage(x) + E.GetDamage(x), Q.DamageType, Q.Range));
                        if (target != null)
                        {
                            E.Cast();
                            Q.Cast(target);
